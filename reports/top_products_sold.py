...
def top_product_sold_generator():
    ...
    create_pivot_and_export(df=df)


def create_pivot_and_export(df, output_file_path='TOP_PRODUCTS_SOLD.xlsx'):
    ...
    for i, type_value in enumerate(df['TYPE'].unique(), start=1):
        ...
        header_style = NamedStyle(name=f'header_style_{i}', fill=PatternFill(start_color=colors_for_sheet[i - 1],
                                                                             end_color=colors_for_sheet[i - 1],
                                                                             fill_type='solid'))
        ...
        for row in worksheet.iter_rows(min_row=1, max_row=16, min_col=1, max_col=3):
            for cell in row:
