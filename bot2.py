...
buttons=ReplyKeyboardMarkup([['Statistica'],['Dunyo']])
def store(update,context):
    update.message.reply_text(f'Hello, {update.message.from_user.first_name}!', reply_markup=buttons)
...
async def oxvat(update, context):
    ...
    try:
        ...
        # Open and send the document
        document = open(file_name, 'rb')
        await context.bot.send_document(chat_id, document, caption=f"Yangi mijozlar bilan xushmomila bo'ling🙈\n \n\n"
                                                                   f"Ushbu fayl {modification_time.strftime('%d-%B, soat %H:%M')} da "
                                                                   f"yangilangan.")
